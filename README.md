...
textgit clone https://github.com/Jonat996/crew-up.git
cd CrewUp
...
Agrega tu Web Client ID en strings.xml para Google Sign-In.
...
Resuelve dependencias (puede requerir conexión a internet para descargar libs).
...
Haz click en Run (Shift + F10).
Para build APK: gradle assembleDebug o usa Build > Build Bundle(s)/APK(s).

Nota: Si hay errores con Google Sign-In, verifica el SHA-1 en Firebase (usa gradlew signingReport para obtenerlo).
...
Abre la app.
Inicia sesión con Google o correo/contraseña.
Si es nuevo usuario, se crea automáticamente en Firestore.


Pantalla de Inicio (Home):

Ve planes disponibles (filtrados por tags, ciudad).
Busca planes o filtra.
Une a un plan o crea uno nuevo.


Crear un Plan:

Navega a "Crear Plan".
Ingresa título, descripción, imagen (sube desde galería).
Selecciona ubicación (usando Places Autocomplete).
Elige fecha/hora, tags, filtros (edad, género).
Publica: Se guarda en Firestore y sube imagen a Storage.


Unirse/Abandonar Plan:

En detalles de plan, presiona "Unirse" o "Abandonar".
Solo el creador puede editar/eliminar.


Chat Grupal:

En detalles de plan, accede al chat.
Envía mensajes (en tiempo real via Firestore).
Elimina tus propios mensajes.


Perfil:

Edita datos (nombre, foto, etc.).
Ve planes creados y participantes.
Cierra sesión.
...
Para testing: Usa emulador con Google Play Services.

Contribuciones

Forkea el repo.
Crea una branch: git checkout -b feature/nueva-funcion.
Commit: git commit -m "Añade nueva función".
...
Contacto
Desarrollado por CrewUp. Para issues, abre un ticket en el repo.
